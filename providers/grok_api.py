"""
Grok API Provider (X.AI) — legacy fallback

The primary Grok provider is now the CLI (providers/grok.py, spawn_grok).
This chat-completions path remains for environments without the Grok CLI,
or for pure-text queries where an agentic run is overkill.
"""
...
def spawn_grok_api(
    ...
    *,
    ...
    resolved_model = settings.get_model_alias("grok-api", model)
    ...
    try:
        ...
        return AgentResult(success=False, text="", spawn_id=spawn_id, error=str(e), provider="grok-api")
